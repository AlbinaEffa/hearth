...
export const api = {
  ...
  rooms: () => req('/rooms'),
  rewardRequests: () => req('/reward-requests'),

  createTask: (b: Record<string, unknown>) => post('/tasks', b),
  takeTask: (id: string) => post(`/tasks/${id}/take`),
  acceptTask: (id: string) => post(`/tasks/${id}/accept`),
  declineTask: (id: string) => post(`/tasks/${id}/decline`),
  ...
  createReward: (b: Record<string, unknown>) => post('/rewards', b),
  requestReward: (b: { target_id: string; title: string; note?: string }) => post('/reward-requests', b),
  approveRewardRequest: (id: string, cost: number) => post(`/reward-requests/${id}/approve`, { cost }),
  declineRewardRequest: (id: string) => post(`/reward-requests/${id}/decline`),
}
