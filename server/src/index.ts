  ...
  try {
    ...
    const u = await c.query(
      `insert into app_user (email, display_name, password_hash)
       values ($1,$2, crypt($3, gen_salt('bf'))) returning id, display_name, avatar_color`,
      [email, name, password],
    )
    const h = await c.query(`insert into household (name, invite_code, created_by) values ($1,$2,$3) returning id`,
      [householdName || `Семья ${name}`, code(), u.rows[0].id])
...
app.get('/members', A, async (req: any) =>
  (await q(`select m.id, m.role, m.is_admin, m.coins, u.display_name as name, u.avatar_color
...
app.post('/tasks', A, async (req: any, reply) => {
  const { title, reward = 0, assignee_id = null, is_personal = false, due_at = null, urgent = false } = req.body || {}
  if (!title) return reply.code(400).send({ error: 'title required' })
  const status = assignee_id ? 'accepted' : 'open'
  const r = await q(
    `insert into task (household_id, title, reward, assignee_id, created_by, status, is_personal, urgent, due_at)
     values ($1,$2,$3,$4,$5,$6,$7,$8,$9) returning *`,
    [req.user.hid, title, reward, assignee_id, req.user.mid, status, is_personal, urgent, due_at])
  return r.rows[0]
})
...
app.post('/rewards/:id/redeem', A, async (req: any, reply) => {
  ...
  try { await rpc('redeem_reward($1,$2)', [member, req.params.id]); return { ok: true } }
    ...
    if (String(e.message).includes('coins_check')) return reply.code(400).send({ error: 'not enough coins' })
    return reply.code(500).send({ error: String(e.message || e) })
  }
})
